...
- **D-01:** `src/hooks/useLunarPhase.js` is a **pure-computation hook** — no fetch, no React Query. It returns `{ phaseName, phaseFraction, dayNightStatus, surfaceTemp, computedAt }`. The hook calls into `src/utils/lunarPhase.js` and re-evaluates **on every render plus once per minute via `useNow()` from Phase 2** (cheap; the math is sub-millisecond and re-rendering every 60s keeps the day/night indicator and phase fraction accurate over long sessions).
